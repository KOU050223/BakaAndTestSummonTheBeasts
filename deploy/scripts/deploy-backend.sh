...
image_repository=${BACKEND_IMAGE_REPOSITORY:-ghcr.io/kou050223/baka-and-test-summon-the-beasts-backend}
deploy_env_file=${DEPLOY_ENV_FILE:-$HOME/.config/baka/backend.env}
deploy_state_dir=${DEPLOY_STATE_DIR:-$HOME/.local/state/baka}
service_name=${BACKEND_SERVICE_NAME:-baka-backend.service}
health_url=${BACKEND_HEALTH_URL:-http://127.0.0.1:8000/up}
health_attempts=${BACKEND_HEALTH_ATTEMPTS:-12}
image="$image_repository:$commit_sha"
current_image=localhost/baka-backend:current
previous_image=localhost/baka-backend:previous
...
podman pull "$image"
podman run --rm --network=host --env-file "$deploy_env_file" "$image" ./bin/rails db:migrate
...
if ((restart_failed == 1)) || ! wait_for_health; then
  ...
  if wait_for_health; then
    printf 'rollback succeeded; previous backend image is running\n' >&2
    exit 1
  fi
...
state_file="$deploy_state_dir/backend-current-sha"
printf '%s\n' "$commit_sha" >"$state_file.tmp"
mv "$state_file.tmp" "$state_file"
printf 'deployed backend commit %s\n' "$commit_sha"
