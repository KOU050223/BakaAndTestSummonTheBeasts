...
SCRIPT="$ROOT_DIR/deploy/scripts/deploy-game.sh"
...
test_deploys_and_records_healthy_image() {
  local temp_dir output status sha state
  temp_dir=$(mktemp -d)
  sha=0123456789abcdef0123456789abcdef01234567
  touch "$temp_dir/game.env" "$temp_dir/commands.log"

  output=$(env \
    PATH="$FAKE_BIN:$PATH" \
    DEPLOY_ENV_FILE="$temp_dir/game.env" \
    ...
    "$SCRIPT" "$sha" 2>&1)
  status=$?
  state=$(cat "$temp_dir/state/game-current-sha" 2>/dev/null || true)

  assert_eq 0 "$status" "healthy deployment succeeds"
  assert_contains "$(cat "$temp_dir/commands.log")" "podman pull ghcr.io/kou050223/baka-and-test-summon-the-beasts-game:$sha" "deployment pulls immutable image"
  assert_contains "$(cat "$temp_dir/commands.log")" "podman tag ghcr.io/kou050223/baka-and-test-summon-the-beasts-game:$sha localhost/baka-game:current" "deployment switches current image"
  assert_contains "$(cat "$temp_dir/commands.log")" "systemctl --user restart baka-game.service" "deployment restarts Quadlet service"
...
test_rolls_back_when_new_image_is_unhealthy() {
  ...
  touch "$temp_dir/game.env" "$temp_dir/commands.log"
  printf '%s\n' "$old_sha" >"$temp_dir/state/game-current-sha"

  output=$(env \
    PATH="$FAKE_BIN:$PATH" \
    DEPLOY_ENV_FILE="$temp_dir/game.env" \
    DEPLOY_STATE_DIR="$temp_dir/state" \
    GAME_HEALTH_ATTEMPTS=1 \
    FAKE_COMMAND_LOG="$temp_dir/commands.log" \
    FAKE_CURRENT_IMAGE_EXISTS=0 \
    ...
    "$SCRIPT" "$new_sha" 2>&1)
  status=$?
  state=$(cat "$temp_dir/state/game-current-sha")
  commands=$(cat "$temp_dir/commands.log")

  assert_eq 1 "$status" "rolled back deployment reports failure"
  assert_contains "$commands" "podman tag localhost/baka-game:previous localhost/baka-game:current" "unhealthy deployment restores previous image"
  assert_count 2 "$commands" "systemctl --user restart baka-game.service" "rollback restarts service a second time"
...
test_rolls_back_when_service_restart_fails() {
  ...
  assert_contains "$commands" "podman tag localhost/baka-game:previous localhost/baka-game:current" "restart failure restores previous image"
  assert_count 2 "$commands" "systemctl --user restart baka-game.service" "restart failure triggers rollback restart"
...
test_stops_unhealthy_first_deployment() {
  ...
  assert_contains "$commands" "systemctl --user stop baka-game.service" "unhealthy first deployment stops service"
...
printf 'all deploy-game tests passed\n'
